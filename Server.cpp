	...
	else if(clients[j].getPassAccepted() == true && clients[j].getRegistered() == false) 
	{
		...
		else if(command == "NICK" || command == "USER")
		{
			...
			else if(!pass.empty()) //check does password matches the server password
			{
				...
				// USER <username> <hostname> <servername> :<realname>  <-- pattern of what client needs to type
				//pattern also used to PRIVMSG
				else if (command == "USER") //checking user with 5 params and real name
				{
					if(tokens.size() < 5)
					{
					...
					else if(token.size() >= 5)
					{
						if(token[4] != ':')
						{
							send(client_fd, unknown_command.c_string(), unknown_command.size(), 0);
							return;
						}
						else(if token[4] == ':')
						{
							clients[j].setUser(token[1]);

						}

						/* 
						if 4th word is not :error need more param/unknown command 
						everything after : is real name with spaces and etc
						if more than 5 parameters before : 
						*/
					}
